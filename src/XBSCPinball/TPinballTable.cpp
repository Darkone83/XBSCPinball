...
#ifdef _XBOX
// RXDK does not provide modern MSVC's __ftol2_sse helper.
// This conversion needs floor(value) semantics, so use the x87 directly:
// temporarily select round-toward-negative-infinity, convert, then restore CW.
static int XbFloorToInt(float value)
{
	int result;
	unsigned short oldControlWord;
	unsigned short floorControlWord;
	...
	__asm
	{
		...
		and ax, 0F3FFh
		or ax, 0400h
		mov floorControlWord, ax
		fldcw floorControlWord
		fld value
...
#ifdef _XBOX
		XbGameRumbleEvent(XB_RUMBLE_TILT);
#endif
...
int TPinballTable::Message(MessageCode code, float value)
{
	...
	case MessageCode::NewGame:
		...
		else
		{
			...
			#ifdef _XBOX
			auto playerCount = XbFloorToInt(value);
#else
