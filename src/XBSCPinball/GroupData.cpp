...
#ifdef _XBOX
		// The desktop build decompresses an ImGui-embedded copy. Xbox deliberately
		// avoids ImGui and loads the original uncompressed blob shipped with source.
		std::string fontPath = pb::make_path_name("PB_MSGFT.bin");
		FILE* fontFile = fopenu(fontPath.c_str(), "rb");
		if (fontFile)
		{
			...
			if (fileSize > 0)
			{
				auto fontData = new uint8_t[(size_t)fileSize];
				if (fread(fontData, 1, (size_t)fileSize, fontFile) == (size_t)fileSize)
					AddMsgFont(reinterpret_cast<MsgFont*>(fontData), "pbmsg_ft");
				delete[] fontData;
			}
			fclose(fontFile);
		}
