...
void gdrv_bitmap8::CreateTexture(const char* scaleHint, int access)
{
	(void)access;
#ifdef _XBOX
	if (Texture) { XbReleaseTexture(Texture); Texture = nullptr; }
	Texture = XbCreateLinearTexture(Width, Height);
	(void)scaleHint;
#else
	if (Texture != nullptr) SDL_DestroyTexture(Texture);
	UsingSdlHint hint{ SDL_HINT_RENDER_SCALE_QUALITY, scaleHint };
	Texture = SDL_CreateTexture(winmain::Renderer, SDL_PIXELFORMAT_BGRA32, access, Width, Height);
...
#ifdef _XBOX
	XbUploadTexture(Texture, BmpBufPtr1, Width, Height, Width * (int)sizeof(ColorRgba));
#else
	int pitch = 0;
	ColorRgba* lockedPixels;
	auto result = SDL_LockTexture(Texture, nullptr, reinterpret_cast<void**>(&lockedPixels), &pitch);
...
#ifdef _XBOX
	XbUploadTexture(bmp.Texture, bmp.BmpBufPtr1, bmp.Width, bmp.Height, bmp.Width * 4);
#else
