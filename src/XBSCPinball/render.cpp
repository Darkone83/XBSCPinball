...
{
    if (!vscreen || !vscreen->Texture) return;
    vscreen->BlitToTexture();
#ifdef _XBOX
    XbBeginFrame();
    const bool linear = options::Options.LinearFiltering;
    if (offset_x == 0 && offset_y == 0)
    {
        XbDrawTexture(vscreen->Texture,
            0.0f, 0.0f, (float)vscreen->Width, (float)vscreen->Height,
            (float)DestinationRect.x, (float)DestinationRect.y,
            (float)(DestinationRect.x + DestinationRect.w),
            (float)(DestinationRect.y + DestinationRect.h), linear);
    }
    else
    {
        const float tableWidthCoef = (float)pb::MainTable->Width / (float)vscreen->Width;
        const float srcSep = (float)vscreen->Width * tableWidthCoef;
        const float dstSep = (float)DestinationRect.w * tableWidthCoef;
        const float dx = offset_x * fullscrn::ScaleX;
        const float dy = offset_y * fullscrn::ScaleY;

        XbDrawTexture(vscreen->Texture,
            0.0f, 0.0f, srcSep, (float)vscreen->Height,
            (float)DestinationRect.x + dx, (float)DestinationRect.y + dy,
            (float)DestinationRect.x + dstSep + dx,
            (float)(DestinationRect.y + DestinationRect.h) + dy, linear);
        XbDrawTexture(vscreen->Texture,
            srcSep, 0.0f, (float)vscreen->Width, (float)vscreen->Height,
            (float)DestinationRect.x + dstSep, (float)DestinationRect.y,
            (float)(DestinationRect.x + DestinationRect.w),
            (float)(DestinationRect.y + DestinationRect.h), linear);
    }
    XbEndFrame();
    ...
    else
    {
        ...
        SDL_Rect srcBoardRect{0,0,srcSeparationX,vscreen->Height};
        SDL_Rect srcSidebarRect{srcSeparationX,0,vscreen->Width-srcSeparationX,vscreen->Height};
        auto dstSeparationX = DestinationRect.w * tableWidthCoef;
        SDL_FRect dstBoardRect{DestinationRect.x + offset_x * fullscrn::ScaleX,
            DestinationRect.y + offset_y * fullscrn::ScaleY, dstSeparationX, (float)DestinationRect.h};
        SDL_FRect dstSidebarRect{DestinationRect.x + dstSeparationX, (float)DestinationRect.y,
            DestinationRect.w-dstSeparationX, (float)DestinationRect.h};
