...
{
	GameInput input{ InputTypes::GameController, logicalButton };
	if (down)
		pb::InputDown(input);
	else
		pb::InputUp(input);
...
/*
 * Fixed Xbox New Game command.
 * Called directly from xb_input.cpp when Y is pressed so this action is not
 * dependent on persisted controller bindings in D:\pinball.ini.
 */
...
#ifdef _XBOX
					char debugLine[512];
					snprintf(debugLine, sizeof(debugLine), "SpaceCadetXB: loading game from %s\n", datFilePath.c_str());
...
void pb::ShowMessageBox(Uint32 flags, LPCSTR title, LPCSTR message)
{
	...
	(void)flags;
	char buffer[1024];
	snprintf(buffer, sizeof(buffer), "SpaceCadetXB: %s\n%s\n", title ? title : "", message ? message : "");
